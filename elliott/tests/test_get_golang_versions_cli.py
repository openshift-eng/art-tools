...
class TestGetGolangVersionsCli(unittest.IsolatedAsyncioTestCase):
    def test_get_golang_versions_advisory(self):
        ...
        flexmock(utillib).should_receive("get_golang_rpm_nvrs").with_args([('runc', 'v1', 'r'), ('podman', 'v1', 'r')], logger).and_return(go_nvr_map)
    ...
    def test_get_golang_versions_nvrs(self):
        ...
        logger = get_golang_versions_cli._LOGGER
        flexmock(utillib).should_receive("get_golang_rpm_nvrs").with_args([('podman', '1.9.3', '3.rhaos4.6.el8')], logger).and_return(go_map_1)
        flexmock(utillib).should_receive("get_golang_container_nvrs").with_args([('podman-container', '3.0.1', '6.el8')], logger).and_return(go_map_2)
        go_map_1.update(go_map_2)
        flexmock(utillib).should_receive("pretty_print_nvrs_go").with_args(go_map_1, report=False)
    ...
    def test_get_golang_versions_release(self, mock_get_nvrs_from_release: AsyncMock):
        ...
        logger = get_golang_versions_cli._LOGGER
        flexmock(rhcos).should_receive("get_container_configs").and_return([{'name': 'machine-os-content'}])
        mock_get_nvrs_from_release.return_value = payload_nvrs
        flexmock(utillib).should_receive("get_golang_container_nvrs").with_args([('ose-cli-container', '3.0.1', '6.el8')], logger).and_return(go_map)
        flexmock(utillib).should_receive("pretty_print_nvrs_go").with_args(go_map, report=False)

        result = runner.invoke(
            cli,
            [
                '--group=openshift-4.14',
                'go',
                '--release',
