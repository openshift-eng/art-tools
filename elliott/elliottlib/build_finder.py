...
class BuildFinder:
    ...
    def __init__(self, koji_api: ClientSession, logger: Optional[Logger] = None) -> None:
        ...
        self._build_cache: Dict[
            str, Optional[Dict]
        ] = {}  # Cache build_id/nvre -> build_dict to prevent unnecessary queries.
    ...
    def from_tag(
        self, build_type: str, tag: str, inherit: bool, assembly: Optional[str], event: Optional[int] = None
    ) -> Dict[str, Dict]:
    ...
    def from_pinned_by_is(
        self, el_version: int, assembly: str, releases_config: Model, rpm_map: Dict[str, RPMMetadata]
    ) -> Dict[str, Dict]:
    ...
    def from_group_deps(self, el_version: int, group_config: Model, rpm_map: Dict[str, RPMMetadata]) -> Dict[str, Dict]:
        ...
        dep_nvrs = {
            ...
            for dep in group_config.dependencies.rpms
            if dep[f"el{el_version}"]
        ...
        if dep_nvrs:
            ...
            self._logger.info(
                "Found %s NVRs defined in group dependencies. Fetching build infos from Brew...", len(dep_nvr_list)
            )
            ...
            art_rpms_in_group_deps = {dep_build["name"] for dep_build in dep_builds} & {
                meta.rpm_name for meta in rpm_map.values()
            }
    ...
    def from_image_member_deps(
        ...
        dep_nvrs = {
            parse_nvr(dep[f"el{el_version}"])["name"]: dep[f"el{el_version}"]
            for dep in meta_config.dependencies.rpms
            if dep[f"el{el_version}"]
        ...
        if dep_nvrs:
            ...
            art_rpms_in_deps = {dep_build["name"] for dep_build in dep_builds} & {
                meta.rpm_name for meta in rpm_map.values()
            }
            if art_rpms_in_deps:
                raise ValueError(
                    f"attachableImage member dependencies cannot have ART managed RPMs: {art_rpms_in_deps}"
                )
            for dep_build in dep_builds:
                component_builds[dep_build["name"]] = dep_build
        return component_builds
    ...
    def from_rhcos_deps(self, el_version: int, assembly: str, releases_config: Model, rpm_map: Dict[str, Dict]):
        ...
        dep_nvrs = {
            parse_nvr(dep[f"el{el_version}"])["name"]: dep[f"el{el_version}"]
            for dep in rhcos_config.dependencies.rpms
            if dep[f"el{el_version}"]
        }
        if dep_nvrs:
            dep_nvr_list = list(dep_nvrs.values())
            self._logger.info(
                "Found %s NVRs defined in RHCOS dependencies. Fetching build infos from Brew...", len(dep_nvr_list)
            )
            dep_builds = self._get_builds(dep_nvr_list)
            missing_nvrs = [nvr for nvr, build in zip(dep_nvr_list, dep_builds) if not build]
            if missing_nvrs:
            ...
            art_rpms_in_rhcos_deps = {dep_build["name"] for dep_build in dep_builds} & {
                meta.rpm_name for meta in rpm_map.values()
            }
