...
class InitShipmentCli:
    def __init__(self, runtime: Runtime, application: str, snapshot: str, stage_rpa: str, prod_rpa: str,
...
@click.option("--advisory-key",
              help="Boilerplate for the advisory releaseNotes. This will be looked up from group's erratatool.yml")
@click.pass_obj
@click_coroutine
async def init_shipment_cli(runtime: Runtime, application: str, snapshot: str, stage_rpa: str,
    ...
    if advisory_key and for_fbc:
        raise ValueError("Cannot use --for-fbc and --advisory-key together. An fbc shipment is not expected to have "
                         "an advisory")

    pipeline = InitShipmentCli(runtime=runtime,
