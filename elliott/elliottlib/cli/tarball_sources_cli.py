...
Use --force to add new tarball sources to an existing directory.".format(os.path.abspath(out_dir))
    ...
    LOGGER.debug("Use working directory {}.".format(os.path.abspath(working_dir)))
    mkdirs(working_dir)
    ...
    for advisory in advisories:
        ...
        builds = tarball_sources.find_builds_from_advisory(advisory, components)
        if not builds:
            yellow_print("No matched builds found from advisory {}. Wrong advisory number?".format(advisory))
            continue
        green_print("Found {} matched build(s) from advisory {}".format(len(builds), advisory))
        for nvr, product, product_version in builds:
            green_print("\t{}\t{}\t{}".format(nvr, product, product_version))
        ...
        for nvr, product, product_version in builds:
            ...
            if out_layout == "flat":
                ...
                nvr_dirs[nvr].add(os.path.join(out_dir, product_version, str(advisory), "release"))
    ...
    click.echo("Fetching build infos for {} from Koji/Brew...".format(", ".join(nvr_dirs.keys())))
    ...
    for build_info in brew_builds:
        ...
        click.echo("Generating tarball source {} for {}...".format(tarball_filename, nvr))
        ...
        with tempfile.NamedTemporaryFile(suffix="-" + tarball_filename, dir=working_dir) as temp_tarball:
            ...
            LOGGER.debug("Temporary tarball file is {}".format(temp_tarball_path))

            tarball_sources.generate_tarball_source(
                temp_tarball, nvr + "/", os.path.join(working_dir, "repos", build_info["name"]), build_info["source"]
            )
            for dest_dir in nvr_dirs[nvr]:
                mkdirs(dest_dir)
                tarball_abspath = os.path.abspath(os.path.join(dest_dir, tarball_filename))
                if os.path.exists(tarball_abspath):
                    yellow_print("File {} will be overwritten.".format(tarball_abspath))

                LOGGER.debug("Copying {} to {}...".format(temp_tarball_path, tarball_abspath))
                ...
                green_print("Created tarball source {}.".format(tarball_abspath))
...
def print_success_message(tarball_sources_list, out_dir):
    relative_paths = [
        os.path.join(os.path.relpath(os.path.dirname(path), out_dir), os.path.basename(path))
