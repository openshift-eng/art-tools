...
@click.option("--comment", "comment", required=False, help="Add comment to bug")
@click.option(
    "--close-placeholder",
    "close_placeholder",
    required=False,
...
@click.pass_obj
def repair_bugs_cli(
    runtime, advisory_id, auto, id, original_state, new_state, comment, close_placeholder, noop, default_advisory_type
):
...
def repair_bugs(bug_ids, original_state, new_state, comment, close_placeholder, noop, bug_tracker: BugTracker):
    ...
    pbar_header(
        "Fetching data for {} bugs: ".format(len(bug_ids)), "Hold on a moment, we have to grab each one", bug_ids
    )
