...
@click.option(
    '--outcome',
    type=click.Choice([e.value for e in KonfluxBuildOutcome]),
    help=f'Limit builds to given OUTCOME {[e.value for e in KonfluxBuildOutcome]}',
)
@click.option('--output', '-o', type=click.Choice(['json']), help='Output in the given format')
@pass_runtime
@click_coroutine
async def find_k_builds_cli(runtime: Runtime, kind, engine, outcome, output):
    ...
    LOGGER.info(
        f'searching for {engine_fmt}builds of kind={kind} in group={runtime.group} with outcome={outcome.value}'
    )
