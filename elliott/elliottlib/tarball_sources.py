...
def find_builds_from_advisory(advisory_number, components):
    ...
    LOGGER.info(
        "Got info for advisory {} - {} - {}: {} - {}".format(
            advisory_number,
            advisory.errata_state,
            advisory.errata_name,
            advisory.synopsis,
            advisory.url(),
        ),
    )
    flattened_builds = [
        BuildWithProductVersion(nvr, advisory._product, product_version) for product_version, nvrs in advisory.errata_builds.items() for nvr in nvrs
    ]
...
def generate_tarball_source(tarball_file, prefix, local_repo_path, source_url, force_fetch=False):
    ...
    if os.path.isdir(local_repo_path) and os.listdir(local_repo_path):
        ...
        discovered_repo = pygit2.discover_repository(
            local_repo_path,
            False,
            os.path.abspath(os.path.dirname(local_repo_path)),
        )
