...
BuildWithProductVersion = collections.namedtuple("BuildWithProductVersion", ["nvr", "product", "product_version"])
...
def find_builds_from_advisory(advisory_number, components):
    ...
    LOGGER.debug("Fetching advisory {} from Errata Tool...".format(advisory_number))

    advisory = errata_tool.Erratum(errata_id=advisory_number)
    LOGGER.info(
        "Got info for advisory {} - {} - {}: {} - {}".format(
            advisory_number, advisory.errata_state, advisory.errata_name, advisory.synopsis, advisory.url()
        )
    )
    flattened_builds = [
        BuildWithProductVersion(nvr, advisory._product, product_version)
        for product_version, nvrs in advisory.errata_builds.items()
        for nvr in nvrs
    ]
    ...
    filtered_builds = [build for build in flattened_builds if matches_components(build)]
...
def generate_tarball_source(tarball_file, prefix, local_repo_path, source_url, force_fetch=False):
    ...
    LOGGER.debug("Source is from repo {}, commit {}".format(source_repo_url, source_commit_hash))

    git_commit = None  # type: pygit2.Commit

    if os.path.isdir(local_repo_path) and os.listdir(local_repo_path):
        LOGGER.debug("Local Git repo {} exists. Examining...".format(local_repo_path))
        ...
        if not discovered_repo:
            raise ValueError("{} exists but is not a valid Git repo.".format(local_repo_path))
        repo = pygit2.Repository(discovered_repo)
        origin = repo.remotes["origin"]  # type: pygit2.Remote
        if origin.url != source_repo_url:
            raise ValueError("Found a different local Git repo in {}".format(discovered_repo))
        LOGGER.info("Use existing local Git repo {}.".format(local_repo_path))
        fetch = force_fetch
        if not force_fetch:
            try:
        ...
        while stack:
            ...
            for _entry in root:
                ...
                if full_name in TARBALL_IGNORES:
                    LOGGER.info("Excluded {} from source tarball.".format(full_name))
                    continue
