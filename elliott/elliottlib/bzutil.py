...
class JIRABug(Bug):
    ...
    def whiteboard_component(self):
        ...
        pscomponent = next((m.group(1) for label in self.bug.fields.labels if (m := p.fullmatch(label))), None)
        if pscomponent:
            return pscomponent
        ...
        return next((m.group(1) for label in self.bug.fields.labels if (m := p.fullmatch(label))), None)
