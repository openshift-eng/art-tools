...
def find_latest_build(builds: List[Dict], assembly: Optional[str]) -> Optional[Dict]:
    ...
    if not assembly:  # if assembly is not enabled, choose the true latest tagged
        ...
        chosen_build = next(
            (build for build in builds if isolate_assembly_in_release(build["release"]) == assembly),
            None,
        )
        if not chosen_build and assembly != "stream":
            # If no such build, fall back to the newest build containing ".assembly.stream"
            chosen_build = next(
                (build for build in builds if isolate_assembly_in_release(build["release"]) == "stream"),
                None,
            )
        if not chosen_build:
            ...
            chosen_build = next(
                (build for build in builds if isolate_assembly_in_release(build["release"]) is None),
                None,
            )
    return chosen_build
