...
from artcommonlib.konflux import konflux_build
...
class KonfluxDb:
    ...
    def _check_env_vars(self):
        ...
        for var in self.REQUIRED_VARS:
            if not os.getenv(var):
                raise RuntimeError(f'Environment variable {var} must be defined')

    @staticmethod
    def _get_column_names() -> str:
        """
        Inspects the KonfluxBuild class definition, and returns its fields a list
        To be used with INSERT statements
        """

        fields = []
        parameters = inspect.signature(konflux_build.KonfluxBuild.__init__).parameters
    ...
    def add_build(self, build: konflux_build.KonfluxBuild):
        """
    ...
    async def add_builds(self, builds: typing.List[konflux_build.KonfluxBuild]):
        """
    ...
    def search_builds_by_fields(self, names: list, values: list):
        ...
        for row in results:
            yield konflux_build.from_result_row(row)
    ...
    def search_build_by_build_id(self, build_id: str) -> typing.Optional[konflux_build.KonfluxBuild]:
        """
        Search for a build matching build ID.
        If no build is found, return None.
        If more than one build is found, raise an error.
        Otherwise, return a KonfluxBuild object
        """
