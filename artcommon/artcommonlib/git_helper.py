...
async def run_git(args: Sequence[str], env: Optional[Dict[str, str]] = None, check: bool = True, **kwargs):
    ...
    :param kwargs: Additional arguments to pass to exectools.cmd_assert_async2
    ...
    if env:
        set_env.update(env)
    return await exectools.cmd_assert_async2(['git'] + list(args), check=check, env=constants.GIT_NO_PROMPTS, **kwargs)


async def gather_git(args: Sequence[str], env: Optional[Dict[str, str]] = None, **kwargs):
    ...
    if env:
        set_env.update(env)
    return await exectools.cmd_gather_async2(['git'] + list(args), env=constants.GIT_NO_PROMPTS, **kwargs)
