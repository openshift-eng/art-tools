...
def analyze_navigation(base_dir: Path, entry_point: Path, max_depth: int = 3) -> Dict:
    ...
    reachable_all = set(depths.keys())
    reachable_expected = expected_docs & reachable_all
    unreachable = expected_docs - reachable_all

    all_docs = expected_docs
    ...
    return {
        ...
        'reachable_docs': len(reachable_expected),
...
def print_report(analysis: Dict, verbose: bool = False):
    ...
    if analysis['over_limit_docs']:
        print(f"DOCS EXCEEDING {analysis['max_depth_limit']} HOPS")
        print("-" * 70)
