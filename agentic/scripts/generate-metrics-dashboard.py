...
Usage:
    python3 agentic/scripts/generate-metrics-dashboard.py
    python3 agentic/scripts/generate-metrics-dashboard.py --output agentic/metrics-dashboard.html
    ...
    for line in output.split('\n'):
            ...
            if 'Status:' in line:
                if current_workflow:
                    ...
                    if match:
                        ...
                        current_workflow = None
            elif line[0].isupper() and not line.startswith(('SUMMARY', 'Workflows', 'PASSED', 'FAILED')):
                current_workflow = {'name': line.strip()}
        ...
        if 'Max observed:' in line:
            ...
            try:
                ...
                pass
        elif 'PASSED' in line and 'workflows' in line.lower():
            metrics['status'] = 'pass'
        elif 'FAILED' in line and 'workflows' in line.lower():
            metrics['status'] = 'fail'
...
<head>
    ...
    <title>art-tools Documentation Metrics Dashboard</title>
    <style>
        * {{ margin: 0; padding: 0; box-sizing: border-box; }}
        ...
        .score-banner {{
            background: {overall_color}; color: white;
        ...
        .score-circle {{
            ...
            flex-direction: column; border: 8px solid rgba(255,255,255,0.3);
        }}
        .score-number {{ font-size: 4rem; font-weight: bold; line-height: 1; }}
        .score-label {{ font-size: 1.5rem; margin-top: 0.5rem; opacity: 0.9; }}
        .metrics-grid {{
        ...
        .metric-card {{
            ...
            padding: 1.5rem; border-left: 4px solid #cbd5e1;
        }}
        .metric-card.pass {{ border-left-color: #10b981; }}
        .metric-card.fail {{ border-left-color: #ef4444; }}
        .metric-card h3 {{ font-size: 1.25rem; margin-bottom: 1rem; color: #1e293b; }}
        .metric-value {{ font-size: 2rem; font-weight: bold; color: #334155; margin-bottom: 0.5rem; }}
        .metric-detail {{ color: #64748b; font-size: 0.9rem; margin-top: 0.5rem; }}
        .progress-bar {{
        ...
        .progress-fill.pass {{ background: linear-gradient(90deg, #10b981, #059669); }}
        .progress-fill.fail {{ background: linear-gradient(90deg, #ef4444, #dc2626); }}
        .detail-section {{ padding: 2rem; border-top: 1px solid #e2e8f0; }}
        .detail-section h2 {{ font-size: 1.5rem; color: #1e293b; margin-bottom: 1rem; }}
        .workflow-item {{
            background: white; padding: 0.75rem; margin-bottom: 0.5rem;
            border-radius: 0.25rem; display: flex;
            justify-content: space-between; align-items: center;
        }}
        .workflow-name {{ font-weight: 500; color: #1e293b; }}
        .workflow-status {{
            padding: 0.25rem 0.75rem; border-radius: 1rem;
            font-size: 0.85rem; font-weight: 500;
        }}
        .workflow-status.pass {{ background: #d1fae5; color: #065f46; }}
        .workflow-status.fail {{ background: #fee2e2; color: #991b1b; }}
        .stats-row {{ display: flex; gap: 2rem; margin-top: 1rem; }}
        .stat {{ flex: 1; }}
        .stat-label {{ color: #64748b; font-size: 0.875rem; margin-bottom: 0.25rem; }}
        .stat-value {{ font-size: 1.5rem; font-weight: bold; color: #1e293b; }}
        .timestamp {{
            text-align: center; padding: 1rem; color: #64748b;
            ...
            <div class="metrics-grid">
                <div class="metric-card {nav_metrics['status']}">
                    ...
                    <div class="metric-detail">
                        ...
                        {'PASSED' if nav_metrics['status'] == 'pass' else 'FAILED'}
                    </div>
                    <div class="metric-detail">
                ...
                <div class="metric-card {budget_metrics['status']}">
                    ...
                    <div class="metric-detail">
                        ...
                        {'PASSED' if budget_metrics['status'] == 'pass' else 'OVER BUDGET'}
                    </div>
                    <div class="metric-detail">
...
{workflow_html}
            ...
            <div class="timestamp">
                Generated on {datetime.now().strftime('%Y-%m-%d %H:%M:%S')} by
                <code>agentic/scripts/generate-metrics-dashboard.py</code>
            </div>
        </div>
    </div>
</body>
