...
class BuildSyncPipeline:
    ...
    def get_permitted_false_assembly_issues(self):
        # file generated by release:gen-payload
        filename = f'{GEN_PAYLOAD_ARTIFACTS_OUT_DIR}/assembly-report.yaml'
        path = os.path.abspath(filename)
        try:
            with open(path) as f:
                report = yaml.safe_load(f.read())
        except FileNotFoundError:
            self.logger.warning('Could not find assembly report file %s', path)
            return None
