"""
open-reconciliation-prs-layered pipeline: Open reconciliation PRs for CI upstream repos (layered products).

Adapted from open-reconciliation-prs for non-OCP groups (layered products
such as logging, oadp, mtc, mta, etc.). Instead of an OCP version, this
pipeline takes a group name directly (e.g. "logging-6.5").
"""
...
class ReconcileCIUpstreamLayeredPipeline:
    """
    Opens reconciliation PRs for CI upstream repos (layered products).

    Adapted from ReconcileCIUpstreamPipeline for non-OCP groups.
    ...
    def __init__(
        ...
        """
        Initialize open-reconciliation-prs-layered pipeline.

        Args:
            runtime: PyARTCD runtime instance
            group: Layered product group to reconcile (e.g., "logging-6.5")
            data_path: ocp-build-data fork URL (default: official repo)
            data_gitref: ocp-build-data git branch/tag/sha (default: use group branch)
    ...
    def _validate_parameters(self) -> None:
        ...
        """
        # Validate group format (required)
        if not self.group:
            raise ValueError("GROUP is required")

        if not re.fullmatch(r'[a-z][a-z0-9-]*-\d+\.\d+', self.group):
            raise ValueError(
                f"Invalid GROUP format: {self.group}. "
                f"Expected format: <product>-<major>.<minor> (e.g., logging-6.5, oadp-1.5, oc-mirror-2.0)"
            )
    ...
    def _working_dir(self) -> str:
        """Get doozer working directory path for this group."""
        return f"{self.runtime.doozer_working}/wd-{self.group}"
    ...
    def _get_gitref(self, group: str) -> str:
        """Get gitref to use: data_gitref if provided, otherwise group name as branch."""
        return self.data_gitref or group

    async def _clone_ocp_build_data(self, group: str) -> Path:
        """
        Clone ocp-build-data repository for specified group.
        ...
        try:
            if self._is_commit_sha(gitref):
                ...
                self._logger.info(f"{group}: Cloning and checking out commit SHA {gitref}")
    ...
    async def _open_reconciliation_prs(self, doozer_opts: str) -> int:
        ...
        """
        self._logger.info(f"{self.group}: Opening reconciliation PRs")
        ...
        for attempt in range(1, self.MAX_RETRIES + 1):
            ...
            if rc == 1 and self.moist_run:
                if last_stderr:
                    self._logger.warning(f"{self.group}: doozer stderr during --moist-run (rc=1): {last_stderr[:500]}")
                self._logger.info(f"{self.group}: PR simulation completed (rc=1 from --moist-run)")
                if attempt > 1:
                    self._logger.info(f"{self.group}: Succeeded on attempt {attempt}")
                return 0
            ...
            if rc == 1 and not self.moist_run and is_transient:
                if attempt < self.MAX_RETRIES:
                    self._logger.warning(
                        f"{self.group}: Transient error on attempt {attempt}/{self.MAX_RETRIES} "
            ...
            if rc == 25:
                self._logger.warning(f"{self.group}: Some PRs skipped (rc=25)")
                if attempt > 1:
                    self._logger.info(f"{self.group}: Succeeded on attempt {attempt}")
                return 25
    ...
    def _cleanup(self, group_dir: Path | None) -> None:
        ...
        if group_dir and group_dir.exists():
            shutil.rmtree(group_dir)
            self._logger.info(f"{self.group}: Cleaned up clone directory")

    async def run(self) -> int:
        """
        Main pipeline: open reconciliation PRs for a single layered product group.
        ...
        """
        jenkins.update_title(f' [{self.group}]')
        self._logger.info(f"Starting open-reconciliation-prs-layered for {self.group}")

        group_dir = None

        try:
            # Clone ocp-build-data
            group_dir = await self._clone_ocp_build_data(self.group)
            ...
            self._logger.error(f"{self.group}: Failed with error: {e}", exc_info=True)
...
@cli.command(
    ...
    help="Open reconciliation PRs for CI upstream repos (layered products). "
...
@click.option(
    ...
    '-g',
    required=True,
    help='Layered product group to reconcile (e.g., "logging-6.5", "oadp-1.5") - REQUIRED.',
)
@click.option(
    '--data-path',
    required=False,
...
@pass_runtime
@click_coroutine
async def open_reconciliation_prs_layered_cli(
    runtime: Runtime,
    group: str,
    data_path: str,
    data_gitref: str,
    ...
    """
    CLI entrypoint for open-reconciliation-prs-layered pipeline.

    Opens reconciliation PRs for CI upstream repos (layered products)
    by running doozer images:streams prs open.
    ...
    pipeline = ReconcileCIUpstreamLayeredPipeline(
        runtime,
        group=group,
        data_path=data_path,
    ...
    # Run with per-group lock
    lock_name = locks.Lock.OPEN_RECONCILIATION_PRS_LAYERED.value.format(group=group)
    lock_id = jenkins.get_build_path_or_random()  # Jenkins build identifier

    try:
        exit_code = await locks.run_with_lock(
            coro=pipeline.run(),
            lock=locks.Lock.OPEN_RECONCILIATION_PRS_LAYERED,
            lock_name=lock_name,
            lock_id=lock_id,
        )
        sys.exit(exit_code if exit_code is not None else 0)
    except Exception:
        runtime.logger.error(f"open-reconciliation-prs-layered failed for {group}", exc_info=True)
        sys.exit(50)
