...
class RebuildPipeline:
    ...
    async def _build_plashets(self, timestamp: str, el_version: int, group_config: Dict, image_config: Optional[Dict]) -> List[PlashetBuildResult]:
        ...
        if self.type == RebuildType.IMAGE:
            ...
            for repo in repos:
                ...
                plashets.append(
                    await self._build_plashet_from_tags(
                        repo, basis_repo_dir, el_version, arches, (plashet_config[1:3],), EMBARGOED_TAGS, signing_advisory
                    )
                )
            ...
            plashets.append(
                await self._build_plashet_for_assembly("plashet-rebuild-overrides", overrides_repo_dir, el_version, arches, signing_advisory)
            )
        elif self.type == RebuildType.RHCOS:
            ...
            plashets.append(
                await self._build_plashet_from_tags(
                    "plashet-rebuild-basis", basis_repo_dir, el_version, arches, (plashet_config[1:3],), EMBARGOED_TAGS, signing_advisory
                )
            )
            # Build "overrides" plashet repo with "for-assembly"
