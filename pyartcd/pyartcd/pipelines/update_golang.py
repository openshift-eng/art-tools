...
class UpdateGolangPipeline:
    def __init__(
        ...
        self.ci_images_refresh = ci_images_refresh
    ...
    async def run(self):
        ...
        if self.ci_images_refresh:
            await self._refresh_ci_images(build_major_minor, allowed_major_minors, el_nvr_map_for_images)
        else:
            _LOGGER.info("--ci-images-refresh not set; skipping CI golang builder/build-root build+sync")
...
@click.option(
    ...
    is_flag=True,
    default=False,
    help='Run the CI golang builder/build-root reconciliation step (rebuild + sync-ci-images). Defaults to False.',
)
@pass_runtime
@click_coroutine
