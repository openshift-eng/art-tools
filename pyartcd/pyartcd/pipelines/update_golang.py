...
class UpdateGolangPipeline:
    ...
    async def run(self):
        ...
        # Check if openshift-golang-builder image builds exist in Konflux for the provided compiler builds
        ...
        if konflux_missing:
            ...
            if konflux_still_missing:
                error_msg = f'Failed to find existing builder(s) for rhel version(s): Konflux: {konflux_still_missing}'
                if self.external_golang_rpms:
                    error_msg += (
...
@click.option(
    '--build-system',
    type=click.Choice(['konflux'], case_sensitive=False),
    default='konflux',
    help='Build system to use for golang-builder images. Only Konflux is supported.',
)
