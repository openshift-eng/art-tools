...
class SyncRhcosBfbPipeline:
    ...
    def build_mirror_destinations(self) -> Tuple[List[str], List[str]]:
        ...
        Returns (versioned_paths, latest_paths) for different update logic.
        """
        if self.sync_type == "bfb":
    ...
    def build_bfb_destinations(self) -> Tuple[List[str], List[str]]:
        ...
        if self.is_prerelease:
            # Pre-release structure: /pre-release/{ocp_version}/{rhcos-build}/
            ...
            # Stable structure: /{major.minor}/{ocp_version}/{rhcos-build}/
    ...
    def add_stable_files_to_artifacts_dir(self):
        ...
        original_files = list(self.artifacts_dir.iterdir())  # Snapshot to avoid iterating over files we're adding
    ...
    async def should_update_global_latest(self) -> bool:
        ...
        For GA'd releases, check {major}.{minor+1}/latest.
        ...
        try:
            result = await exectools.cmd_assert_async(["aws", "s3", "ls", path_to_check])
            print(f"Results of `aws s3 ls`: \n{result}")

            # If command succeeds and has non-empty output, next version exists
            return not bool(result.stdout.strip())

        except Exception:
            # If command fails, next version doesn't exist, so we should update
            return True
...
@click.option("--type", "sync_type", default="bfb", help="Sync type: 'bfb' for NVIDIA BFB artifacts (default)")
@pass_runtime
@click_coroutine
