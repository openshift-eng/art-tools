...
from pyartcdlib.pipelines.rebuild import PlashetBuildResult, RebuildPipeline, RebuildType
...
class TestRebuildPipeline(IsolatedAsyncioTestCase):
    ...
    async def test_build_plashet_from_tags(
        self, cmd_assert_async: AsyncMock, rmtree: Mock, path_exists: Mock, path_mkdir: Mock
    ):
        runtime = MagicMock(
            config={"build_config": {"ocp_build_data_url": "https://example.com/ocp-build-data.git"}},
        ...
        actual = await pipeline._build_plashet_from_tags(
    ...
    @patch("shutil.rmtree")
    @patch("artcommonlib.exectools.cmd_assert_async")
    async def test_build_plashet_for_assembly_rhcos(
        self, cmd_assert_async: AsyncMock, rmtree: Mock, path_exists: Mock, path_mkdir: Mock
    ):
        runtime = MagicMock(
            config={"build_config": {"ocp_build_data_url": "https://example.com/ocp-build-data.git"}},
    ...
    @patch("shutil.rmtree")
    @patch("artcommonlib.exectools.cmd_assert_async")
    async def test_build_plashet_for_assembly_image(
        self, cmd_assert_async: AsyncMock, rmtree: Mock, path_exists: Mock, path_mkdir: Mock
    ):
        runtime = MagicMock(
            config={"build_config": {"ocp_build_data_url": "https://example.com/ocp-build-data.git"}},
    ...
    async def test_copy_plashet_out_to_remote(self, cmd_assert_async: AsyncMock):
        ...
        cmd_assert_async.assert_any_await(
            [
                ...
                "--perms",
                "--",
                "/path/to/local/plashets/el8/plashet1234",
        ...
        cmd_assert_async.assert_any_await(
            [
                ...
                "--",
                "plashet1234",
                f"{constants.PLASHET_REMOTE_BASE_DIR}/4.9-el8/art0001/building",
            ]
        )
    ...
    async def test_build_plashets_rhcos(
        self, _build_plashet_from_tags: AsyncMock, _build_plashet_for_assembly: AsyncMock
    ):
        runtime = MagicMock(dry_run=False)
        group_config = {
            "arches": ["x86_64", "s390x"],
        ...
        _build_plashet_for_assembly.return_value = PlashetBuildResult(
            "plashet2", Path("/path/to/local/dir2"), "https://example.com/dir2"
        )
    ...
    @patch("pyartcd.pipelines.rebuild.RebuildPipeline._build_plashet_for_assembly")
    @patch("pyartcd.pipelines.rebuild.RebuildPipeline._build_plashet_from_tags")
    async def test_build_plashets_image(
        self, _build_plashet_from_tags: AsyncMock, _build_plashet_for_assembly: AsyncMock
    ):
        runtime = MagicMock(dry_run=False)
        group_config = {
    ...
    def test_generate_repo_file_for_image(self, read_text: Mock):
        ...
        plashets = [
            PlashetBuildResult(
                "rhel-8-server-ose", "fake-basis", "https://example.com/plashets/4.9-el8/art0001/fake-basis"
            ),
            PlashetBuildResult(
                "plashet-rebuild-basis2", "fake-basis2", "https://example.com/plashets/4.9-el8/art0001/fake-basis2"
            ),
            PlashetBuildResult(
                "plashet-rebuild-overrides",
                "fake-overrides",
                "https://example.com/plashets/4.9-el8/art0001/fake-overrides",
            ),
        ]
...
priority = 1
    ...
    def test_generate_repo_file_for_rhcos(self):
        ...
        plashets = [
            PlashetBuildResult(
                "plashet-rebuild-basis", "fake-basis", "https://example.com/plashets/4.9-el8/art0001/fake-basis"
            ),
            PlashetBuildResult(
                "plashet-rebuild-overrides",
                "fake-overrides",
...
images:
  foo:
    ...
    async def test_rebase_and_build_rpm(self, cmd_assert_async: Mock, open: Mock):
        ...
        self.assertEqual(
            nvrs, ["foo-v1.2.3-202107160000.p0.assembly.art0001.el8", "foo-v1.2.3-202107160000.p0.assembly.art0001.el7"]
        )
        open.assert_called_once_with(runtime.working_dir / "doozer-working/record.log", "r")
    ...
    async def test_run_image(
        ...
        load_group_config: AsyncMock,
        load_releases_config: AsyncMock,
        _get_meta_config: AsyncMock,
        _build_plashets: AsyncMock,
        _rebase_image: AsyncMock,
        _generate_repo_file_for_image: Mock,
        _copy_plashet_out_to_remote: AsyncMock,
        _build_image: AsyncMock,
        _generate_example_schema: Mock,
    ):
        mock_datetime.utcnow.return_value = datetime(2021, 7, 16, 0, 0, 0, 0, tzinfo=timezone.utc)
        ...
        _generate_repo_file_for_image.assert_called_once_with(
            open.return_value.__enter__.return_value, plashets, group_config["arches"]
        )
