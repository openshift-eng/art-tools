...
class TestGenAssemblyPipeline(IsolatedAsyncioTestCase):
    ...
    def test_get_nightlies(self, cmd_gather_async: AsyncMock):
        ...
        pipeline = GenAssemblyPipeline(
            runtime, "openshift-4.12", "4.12.99", "brew",
        ...
        cmd_gather_async.assert_awaited_once_with(
            ['doozer', '--group', 'openshift-4.12', '--assembly', 'stream', '--build-system', 'brew', 'get-nightlies'], stderr=None, env=ANY,
        )
        ...
        cmd_gather_async.assert_awaited_once_with(
            [
                'doozer', '--group', 'openshift-4.12', '--assembly', 'stream', '--build-system', 'brew', 'get-nightlies', '--allow-pending',
                '--allow-rejected', '--allow-inconsistency',
            ], stderr=None, env=ANY,
        )
        ...
        cmd_gather_async.assert_awaited_once_with(
            [
                'doozer', '--group', 'openshift-4.12', '--assembly', 'stream', '--build-system', 'brew', '--arches', 'x86_64,aarch64',
...
releases:
  4.12.99:
    ...
    @patch(
        "pyartcd.pipelines.gen_assembly.GenAssemblyPipeline._create_or_update_pull_request", autospec=True,
    ...
    async def test_run(
        self, get_nightlies: AsyncMock, _gen_assembly_from_releases: AsyncMock,
