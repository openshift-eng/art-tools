...
class TestUpdateGolangPipeline(IsolatedAsyncioTestCase):
    ...
    @patch("pyartcd.pipelines.update_golang.KonfluxDb")
    def test_test_assembly_requires_unified_golang_branch(self, mock_konflux_db):
        with self.assertRaisesRegex(ValueError, "test assembly requires the unified golang branch"):
            UpdateGolangPipeline(
                runtime=self._make_test_runtime(),
                ocp_version="4.16",
                cves=None,
...
class TestBuildGolangPlashets(IsolatedAsyncioTestCase):
    ...
    def _make_pipeline(self, use_new_golang_branch=False, dry_run=False, assembly=DEFAULT_GOLANG_ASSEMBLY):
        mock_slack = Mock()
        mock_slack.say_in_thread = AsyncMock()
    ...
    async def test_test_assembly_triggers_test_plashet(self, mock_konflux_db, mock_jenkins):
        pipeline = self._make_pipeline(use_new_golang_branch=True, assembly="test")
        mock_jenkins.start_build_plashets.return_value = "SUCCESS"
