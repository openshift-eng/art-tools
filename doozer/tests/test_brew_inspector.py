...
class TestBrewBuildRecordInspector(unittest.TestCase):
    ...
    def test_info_parsing(self, fake_cmd_assert):
        ...
        image_archives_list_rpms = yaml.safe_load(
            image_res.joinpath("container_image_archive_rpms.yaml").read_text()
        )  # dict[imageID] => koji.listRPMs(imageID)
        image_archives_builds_used = yaml.safe_load(
            image_res.joinpath("build_dicts.yaml").read_text()
        )  # dict[buildID] => koji.getBuild(buildID)
        ...
        self.assertEqual(
            bbii.get_labels()["io.openshift.build.commit.url"],
            "https://github.com/openshift/sriov-network-operator/commit/51a98df728d6712c469946b6ed5973c2fba1454e",
        )
        ...
        for ai in archive_inspectors:
            self.assertIn(
                ai.get_archive_id(),
                image_archives_list_rpms.keys(),
                "ArchiveIDs not reported corrected from abstraction class",
            )
        ...
        self.assertIsNone(
            bbii.get_image_inspector("s390x").get_installed_package_build_dicts().get("tzdata", None)
        )  # Remove this from the resource data by hand for this test
        self.assertIsNotNone(
            bbii.get_image_inspector("x86_64").get_installed_package_build_dicts().get("tzdata", None)
        )  # Remove this from the resource data by hand for this test
        self.assertIsNotNone(
            bbii.get_all_installed_package_build_dicts().get("tzdata", None)
        )  # Even though not in s390x, it should be in aggregate
        self.assertEqual(
            bbii.get_all_installed_package_build_dicts()["grep"]["nvr"], "grep-3.1-6.el8"
        )  # Ensure aggregate has the same nvr for grep

        self.assertEqual(
            bbii.get_image_inspector("s390x").get_pullspec(),
            "registry-proxy.engineering.redhat.com/rh-osbs/openshift-ose-sriov-operator-must-gather:rhaos-4.9-rhel-8-containers-candidate-98861-20210726154614-s390x",
        )
        self.assertEqual(
            bbii.get_image_inspector("s390x").get_digest(),
            "sha256:1f3ebef02669eca018dbfd2c5a65575a21e4920ebe6a5328029a5000127aaa4b",
        )
