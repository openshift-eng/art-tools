...
class TestPackagesFromContainerfile(unittest.TestCase):
    ...
    @patch("doozerlib.lockfile_prototype.resolver.cmd_gather_async")
    def test_packages_from_containerfile_mounts_source_context(self, mock_gather):
        """
        The Containerfile source directory must be available to the
        containerfile package extractor for copied scripts and package lists.
        """
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            source_dir.mkdir()
            containerfile = source_dir / "Dockerfile"
            containerfile.write_text(
                "FROM base\n"
                "COPY install-deps.sh /tmp/\n"
                "RUN /tmp/install-deps.sh\n"
            )
            (source_dir / "install-deps.sh").write_text("dnf install -y test-package\n")
            ...
            async def mock_cmd(cmd, **kwargs):
                ...
                host_tmpdir = work_mount.split(":", 1)[0]
            ...
            mock_gather.side_effect = mock_cmd
            resolver = RpmResolver(working_dir=Path(tmpdir))
            config = RpmsInConfig(arches=["x86_64"], contentOrigin={"repos": []}, packages=[])
            asyncio.run(
                resolver.resolve(
                    config,
        ...
        self.assertEqual(captured_configs[0]["packagesFromContainerfile"], {
            "file": "context/Dockerfile",
            "stageNum": 0,
        })
        context_mounts = [
            captured_cmds[0][i + 1]
            for i, arg in enumerate(captured_cmds[0][:-1])
            if arg == "-v"
        ]
        self.assertIn(
            f"{source_dir}:/work/context:ro,Z",
            context_mounts,
        )
    ...
    def test_packages_from_containerfile_injected_into_config(self, mock_gather):
        ...
        # Container-side path — the host source directory is mounted at /work/context.
        self.assertEqual(pfc["file"], "context/Dockerfile")
        self.assertEqual(pfc["stageNum"], 2)

        # Host Containerfile source directory must be mounted into the container
        cmd = captured_cmds[0]
        expected_mount = "/path/to:/work/context:ro,Z"
        self.assertTrue(
            any(arg == "-v" and cmd[i + 1] == expected_mount for i, arg in enumerate(cmd) if i + 1 < len(cmd)),
