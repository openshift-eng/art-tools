...
class TestDetectEmbargoCli(TestCase):
    ...
    def test_detect_embargoes_in_tags(self):
        ...
        included_builds = [
            [
                {"id": 11, "nvr": "foo11-1.2.3-1.p0"},
                {"id": 12, "nvr": "foo12-1.2.3-1.p1"},
                {"id": 13, "nvr": "foo13-1.2.3-1.p1"},
            ],
            [
                {"id": 21, "nvr": "foo21-1.2.3-1.p0"},
                {"id": 22, "nvr": "foo22-1.2.3-1.p1"},
                {"id": 23, "nvr": "foo23-1.2.3-1.p1"},
            ],
        ]
        excluded_tags = ["a", "b"]
        excluded_builds = [
        ...
        with (
            ...
            patch(
                "doozerlib.build_status_detector.BuildStatusDetector.find_embargoed_builds", return_value=[13, 23]
            ) as find_embargoed_builds,
        ):
    ...
    def test_detect_embargoes_in_pullspecs(self, parallel_exec):
        ...
        with patch(
            "doozerlib.cli.detect_embargo.detect_embargoes_in_nvrs", return_value=[builds[1]]
        ) as detect_embargoes_in_nvrs:
