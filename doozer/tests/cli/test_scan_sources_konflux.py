...
class TestFilterParentInheritedRpms(TestScanSourcesKonflux):
    """Tests for _filter_parent_inherited_rpms with member and stream parents."""

    def setUp(self):
        super().setUp()
        # Mock image_meta with a configurable from.member / from.stream
        self.image_meta.distgit_key = 'child-image'
        self.image_meta.config = MagicMock()

        # Sample non-latest RPMs detected in the child image
        self.non_latest_rpms = {
            'x86_64': [
                ('dmidecode-1:3.3-7.el9.x86_64', 'dmidecode-1:3.3-7.el9_2.1.x86_64', 'baseos'),
                ('bash-0:5.1.8-6.el9.x86_64', 'bash-0:5.1.8-9.el9.x86_64', 'baseos'),
            ],
        }

    async def test_member_parent_filters_inherited_rpms(self):
        """Member parent RPMs that match outdated RPMs are suppressed."""
        self.image_meta.config.__getitem__ = MagicMock(return_value=MagicMock(member='parent-image', stream=None))

        parent_build = MagicMock(spec=KonfluxBuildRecord)
        self.scanner.latest_image_build_records_map['parent-image'] = parent_build

        # Parent has dmidecode at the same (outdated) NEVRA
        parent_rpms = [
            {'name': 'dmidecode', 'epoch': '1', 'version': '3.3', 'release': '7.el9', 'arch': 'x86_64'},
        ]
        self.scanner.package_rpm_finder.get_brew_rpms_from_build_record = MagicMock(return_value=parent_rpms)

        result = await self.scanner._filter_parent_inherited_rpms(self.image_meta, self.non_latest_rpms)

        # dmidecode should be filtered out (inherited from parent), bash should remain
    ...
    async def test_member_parent_no_build_record_returns_unfiltered(self):
        """When the member parent has no build record, all RPMs are returned."""
        self.image_meta.config.__getitem__ = MagicMock(return_value=MagicMock(member='parent-image', stream=None))
        # No parent build in the map
        self.scanner.latest_image_build_records_map.pop('parent-image', None)

        result = await self.scanner._filter_parent_inherited_rpms(self.image_meta, self.non_latest_rpms)

        self.assertEqual(result, self.non_latest_rpms)

    async def test_no_parent_returns_unfiltered(self):
        """When there is no member or stream parent, all RPMs are returned."""
        self.image_meta.config.__getitem__ = MagicMock(return_value=MagicMock(member=None, stream=None))
    ...
    async def test_stream_parent_filters_inherited_rpms(self, mock_cmd_gather):
        """Stream parent RPMs that match outdated RPMs are suppressed."""
        self.image_meta.config.__getitem__ = MagicMock(return_value=MagicMock(member=None, stream='rhel9'))

        # Mock resolve_stream to return a stream config with an image pullspec
        ...
        # dmidecode should be filtered out (inherited from stream parent), bash should remain
        self.assertIn('x86_64', result)
        self.assertEqual(len(result['x86_64']), 1)
        self.assertEqual(result['x86_64'][0][0], 'bash-0:5.1.8-6.el9.x86_64')
    ...
    async def test_stream_parent_resolve_failure_returns_unfiltered(self):
        """When resolve_stream fails, all RPMs are returned."""
        self.image_meta.config.__getitem__ = MagicMock(return_value=MagicMock(member=None, stream='nonexistent'))

        self.runtime.resolve_stream = MagicMock(side_effect=IOError("stream not found"))
    ...
    @patch('doozerlib.cli.scan_sources_konflux.cmd_gather_async', new_callable=AsyncMock)
    async def test_stream_parent_all_rpms_inherited_returns_empty(self, mock_cmd_gather):
        """When all outdated RPMs are inherited from the stream parent, result is empty."""
        self.image_meta.config.__getitem__ = MagicMock(return_value=MagicMock(member=None, stream='rhel9'))
        ...
        # All RPMs inherited — filtered dict should be empty
        self.assertEqual(result, {})

    @patch('doozerlib.cli.scan_sources_konflux.cmd_gather_async', new_callable=AsyncMock)
    async def test_stream_parent_epochnum_zero_for_no_epoch(self, mock_cmd_gather):
        """Packages without an explicit epoch use 0 (via %{EPOCHNUM}), matching to_nevra()."""
        self.image_meta.config.__getitem__ = MagicMock(return_value=MagicMock(member=None, stream='rhel9'))
        ...
        # non_latest_rpms: bash has epoch 0 (no explicit epoch) — to_nevra() outputs "0:"
        no_epoch_rpms = {
            'x86_64': [
                ('bash-0:5.1.8-6.el9.x86_64', 'bash-0:5.1.8-9.el9.x86_64', 'baseos'),
            ],
        ...
        # bash should be filtered — the 0: epoch from %{EPOCHNUM} matches to_nevra() output
        self.assertEqual(result, {})

    @patch('doozerlib.cli.scan_sources_konflux.cmd_gather_async', new_callable=AsyncMock)
    async def test_stream_parent_multi_arch_queries_each_arch(self, mock_cmd_gather):
        """Stream parent is queried per-arch so that arch-specific NEVRAs match correctly."""
        self.image_meta.config.__getitem__ = MagicMock(return_value=MagicMock(member=None, stream='rhel9'))
        ...
        # non_latest_rpms spanning two arches
        multi_arch_rpms = {
            'x86_64': [
                ('dmidecode-1:3.3-7.el9.x86_64', 'dmidecode-1:3.3-7.el9_2.1.x86_64', 'baseos'),
            ],
            'aarch64': [
                ('dmidecode-1:3.3-7.el9.aarch64', 'dmidecode-1:3.3-7.el9_2.1.aarch64', 'baseos'),
            ],
        }
        ...
        result = await self.scanner._filter_parent_inherited_rpms(self.image_meta, multi_arch_rpms)
    ...
    async def test_member_parent_takes_precedence_over_stream(self):
        ...
        self.image_meta.config.__getitem__ = MagicMock(return_value=MagicMock(member='parent-image', stream='rhel9'))

        parent_build = MagicMock(spec=KonfluxBuildRecord)
        self.scanner.latest_image_build_records_map['parent-image'] = parent_build

        # Parent has no matching RPMs → nothing filtered
        self.scanner.package_rpm_finder.get_brew_rpms_from_build_record = MagicMock(return_value=[])

        result = await self.scanner._filter_parent_inherited_rpms(self.image_meta, self.non_latest_rpms)

        # No RPMs inherited from member parent, so all should remain
        self.assertEqual(result, self.non_latest_rpms)
        # resolve_stream should NOT have been called
        self.runtime.resolve_stream.assert_not_called()
