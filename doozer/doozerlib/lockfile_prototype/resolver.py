...
class RpmResolver:
    ...
    def _build_podman_cmd(
        ...
        Arg(s):
            ...
            containerfile_path (str | None): Host path to the Containerfile.
                Its parent directory is mounted read-only at /work/context
                so package lists and scripts referenced by the Containerfile
                are available to the extractor.
        Return Value(s):
        ...
        # Containerfile source context — mounted read-only so
        # packagesFromContainerfile can read copied scripts and package lists.
        if containerfile_path:
            source_dir = Path(containerfile_path).parent
            cmd.extend(["-v", f"{source_dir}:/work/context:ro,Z"])
    ...
    async def resolve(
        ...
        """
        if containerfile_path:
            # Use the container-side path — the Containerfile source context
            # is mounted at /work/context by _build_podman_cmd. The path is
            # relative to the input file at /work/rpms.in.yaml.
            containerfile_name = Path(containerfile_path).name
            pfc_spec: dict = {"file": f"context/{containerfile_name}"}
