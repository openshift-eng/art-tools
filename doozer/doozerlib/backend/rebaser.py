...
class KonfluxRebaser:
    ...
    async def _resolve_image_from_upstream_parent(self, original_parent: str, dfp: DockerfileParser) -> Optional[str]:
        ...
        try:
            # Quick path: Try to extract version info directly from pullspec
            major = None
            minor = None
            el_version = None

            if ':' in original_parent:
                tag = original_parent.split(':')[-1]
                # Extract RHEL version (e.g., rhel-9 -> 9)
                rhel_match = re.search(r'(?:rhel|ubi|centos)-?(\d+)', tag)
                if rhel_match:
                    el_version = int(rhel_match.group(1))
                # Extract golang version (e.g., golang-1.22 -> major=1, minor=22)
                golang_match = re.search(r'golang-(\d+)\.(\d+)', tag)
                if golang_match:
                    major = int(golang_match.group(1))
                    minor = int(golang_match.group(2))

            # If we couldn't extract from pullspec, fall back to oc image info
            if major is None or minor is None or el_version is None:
                self._logger.debug('Retrieving image info for image %s', original_parent)
                labels = (await util.oc_image_info_for_arch_async__caching(original_parent))['config']['config'][
                    'Labels'
                ]
