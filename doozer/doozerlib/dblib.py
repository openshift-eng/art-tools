...
class DB(object):
    ...
    def select(self, expr, limit=100):
        ...
        exeresult = []
        db_connection = mysql_connector.connect(host=self.host, user=self.db_user, password=self.pwd, database=self.db)
    ...
    def check_database_exists(self):
        ...
        db_check_connection = mysql_connector.connect(host=self.host, user=self.db_user, password=self.pwd)

        cursor = db_check_connection.cursor()
        cursor.execute("select count(*) from INFORMATION_SCHEMA.SCHEMATA where SCHEMA_NAME='{}'".format(self.db))

        exception_raised = False

        if cursor.fetchone()[0] != 0:
            self.runtime.logger.info("Configured database [{}] present on MySQL host.".format(self.db))
            return True
        else:
    ...
    def handle_missing_table(self, table_name):
        ...
        if not table_exist_status:
            ...
            self.runtime.logger.info("Successfully create table [{}] in database [{}].".format(table_name, self.db))
                ...
                try:
                    ...
                    self.runtime.logger.info(
                        "Added new column [{}] of identified type [{}] to table [{}] of database [{}].".format(
                            missing_column, column_type, table_name, self.db
                        )
                    )
                except Exception as e:
                    self.runtime.logger.warning(
                        "Could not add new column [{}] of identified type [{}] to table [{}] "
                        "of database [{}]. Error: {}".format(missing_column, column_type, table_name, self.db, e)
                    )
    ...
    def insert_payload(self, payload, table_name, dry_run=False):
        ...
        try:
            ...
            self.runtime.logger.error(
                "Something went wrong creating payload entry in the database. Payload is {}. Exception is {}.".format(
                    payload, e
                ),
                exc_info=True,
            )
