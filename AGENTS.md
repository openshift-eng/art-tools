...
For long-term work (features, stories, epics), implementation plans must be reviewed and approved in the **art-plans** repository before code execution begins. This ensures the team agrees on the approach before AI-generated code is produced.

### When to Suggest a Plan

Before starting implementation, ask the user: *"This looks like long-term work. Should I create a plan document in art-plans for team review before starting implementation?"*

Suggest a plan when:
- The work is long-term or multi-session (features, stories, epics)
- The change introduces new classes, modules, or significant architectural shifts
- The change modifies public interfaces or return types across multiple components

Do NOT suggest a plan for:
- Ops work (incident response, hotfixes, routine maintenance)
- Short-term tasks (bug fixes, typos, test additions, config tweaks)
- Single-session work that can be reviewed directly in the code PR

### Workflow

1. Check if a local clone of `art-plans` exists in the workspace. If not, prompt the user to fork the repo (https://github.com/openshift-eng/art-plans) to their account and clone it locally.
2. Draft the plan following the format in `art-plans/AGENTS.md` and the template at `art-plans/art-tools/TEMPLATE.md`.
3. Raise a PR to art-plans with the plan document in the `art-tools/` directory.
4. Wait for team approval (PR merge) before starting implementation.
5. Once approved, read the merged plan and follow the Agent Execution Details section.

### Plan Document Location

Plans for this repository go in: `art-plans/art-tools/<JIRA-KEY>-<short-slug>.md`

### Executing an Approved Plan

When the user provides an approved plan (or points to one in art-plans), read it and follow the **Agent Execution Details** section. The human review section provides context on intent; the agent section provides precise instructions.
